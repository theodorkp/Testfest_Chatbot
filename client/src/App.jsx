...
const suggestedPrompts = [
  "Hva er forskjellen på WCAG og universell utforming?",
  "Hva er de vanligste feilene som blir funnet under en testfest?",
  "Hvordan kan vi forbedre tilgjengeligheten i et skjema?",
  "Hva bør vi prioritere først hvis tjenesten vår har flere UU-feil?",
];
...
export default function App() {
  ...
  const theme = {
    pageBg: isDarkMode ? "bg-slate-950 text-slate-100" : "bg-slate-100 text-slate-900",
    headerBg: isDarkMode ? "bg-slate-900" : "bg-white",
    panelBg: isDarkMode ? "bg-slate-900" : "bg-white",
    mutedCard: isDarkMode ? "bg-slate-800 text-slate-100" : "bg-slate-100 text-slate-900",
    userCard: "bg-blue-600 text-white",
    subText: isDarkMode ? "text-slate-300" : "text-slate-600",
    border: isDarkMode ? "border-slate-700" : "border-slate-200",
    inputBg: isDarkMode ? "bg-slate-900 text-slate-100 border-slate-700" : "bg-white text-slate-900 border-slate-300",
    ...
    toggleKnob: isDarkMode ? "translate-x-5 bg-blue-500" : "translate-x-0 bg-white",
  };
    ...
    />
            ...
            {showSettingsMenu && (
              ...
              >
                ...
                <div className={`mt-4 border-t pt-3 text-xs ${theme.border} ${theme.subText}`}>
                </div>
              </div>
            )}
          </div>
        ...
        <section className={`flex flex-1 flex-col rounded-2xl shadow-sm ${theme.panelBg}`}>
          <div className="flex-1 space-y-4 overflow-y-auto p-4 md:p-6">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`max-w-3xl rounded-2xl px-4 py-3 text-sm leading-6 ${
                  msg.role === "user" ? `ml-auto ${theme.userCard}` : theme.mutedCard
                }`}
              >
            ...
            {loading && (
              <div className={`max-w-3xl rounded-2xl px-4 py-3 text-sm ${theme.mutedCard}`}>
                Tenker …
              </div>
            )}
          </div>

          <form onSubmit={handleSubmit} className={`border-t p-4 md:p-6 ${theme.border}`}>
